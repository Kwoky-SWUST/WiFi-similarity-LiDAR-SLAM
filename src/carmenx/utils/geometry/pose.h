...
// --------------------------------------------------------------------------
{
  public:
    ...
    Pose( const Matrix& m );    ///<  Create this pose from the given homogeneous vector (4x1 matrix) or homogeneous matrix (4x4)
    ...
    /** Convert the 3D position (i.e., only x/y/z) to a homogeneous
     *  vector (4x1 matrix)
     */
    Matrix getHomogeneousPositionVector() const;

    /** Convert the 3D position and orientation to a 6D vector (6x1 matrix) */
    Matrix as6DVector() const;
