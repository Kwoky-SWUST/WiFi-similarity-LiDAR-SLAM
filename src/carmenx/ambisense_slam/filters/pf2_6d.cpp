...
// --------------------------------------------------------------------------
void
pose2matrix( const Pose&p, Matrix& M )
// --------------------------------------------------------------------------
{
  assert( M.Nrows() == 6 );
  M(1,1) = p.x;
...
// --------------------------------------------------------------------------
carmen_ambisense_particle_ipc_t pf2::Particle6D::
    asIPCParticle() const
// --------------------------------------------------------------------------
{
...
// --------------------------------------------------------------------------
{
  GaussNumberPair< double > pair, pair_z;
  ...
  if ( orientationStdDev > 0 )
  {
    for ( uint i = 0; i < particles.size(); ++i )
    {
      ...
      //particles[i].theta_z += pair.value[i % 2] * orientationStdDev;
      particles[i].theta_z += pair.value[i & 1] * orientationStdDev;
    }
  }
}
...
// --------------------------------------------------------------------------
{
  ...
  for ( std::vector< Particle6D >::const_iterator it = particles.begin(); it != particles.end(); ++it )
  {
    pose2matrix( *it, X );
    D  = X - Mean;
    D(4,1) = carmen_normalize_theta( D(4,1) );
    D(5,1) = carmen_normalize_theta( D(5,1) );
    D(6,1) = carmen_normalize_theta( D(6,1) );
    C += D * D.t() * it->weight;
  }
