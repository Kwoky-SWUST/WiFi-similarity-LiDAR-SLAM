...
public:
	...
	std::map<std::string, vector<int>> wifi_scan_extended;
...
int main(int argc, char **argv)
{
		...
		for (iterator_search = m_radio_fingerprints.begin(); iterator_search != m_radio_fingerprints.end(); ++iterator_search) // 每次都遍历所有的WiFi指纹 根据timestamp来切割
		{
			...
			if (time_diff <= m_extend_time_threshold)
			{
				...
				for (it_scan = temp_wifi_scan.begin(); it_scan != temp_wifi_scan.end(); ++it_scan)
				{
					...
					if (max_RSS_num < v_wifi_scan[it_scan->first].size())
					{
						max_RSS_num = v_wifi_scan[it_scan->first].size();
					}
				}
			}
		}
	...
	std::map<std::string, int> mac_graph;
	int mac_num = 0;
	//int abc = 0;
	std::map<std::string, int>::iterator it_iterator_map;
	// %%%修改ing:
	// 把wifi的mac地址和 在图像中所处的横坐标进行映射 并存储
	for (it_current_fingerprint = m_radio_fingerprints.begin(); it_current_fingerprint != m_radio_fingerprints.end(); it_current_fingerprint++)
	{
		//std::map<long int, MobileScanning>::iterator it_fingerprint_check;
		
		// std::map<std::string, int> current_map = it_current_fingerprint->second.wifi_scan;
		//std::cout<< abc <<endl;
		for(it_iterator_map = it_current_fingerprint->second.wifi_scan.begin(); 
		 it_iterator_map != it_current_fingerprint->second.wifi_scan.end(); it_iterator_map++)
		{
			if(mac_num == 0)
			{
				mac_graph[it_iterator_map->first] = mac_num;
				std::cout<< "mac地址" << it_iterator_map->first << "对应的横坐标为" << mac_num << std::endl;
				mac_num = 1;
			}
			else if(mac_graph.find(it_iterator_map->first) == mac_graph.end())
			{
				mac_graph[it_iterator_map->first] = mac_num;
				std::cout<< "mac地址" << it_iterator_map->first << "对应的横坐标为" << mac_num << std::endl;
				mac_num ++;
	...
	// std::map<std::string, std::vector<int>> wifi_scan_temp;
	// std::map<int,cv::Mat> timestamp_cvmat;
	// cv::Mat RSS_pic(mac_num,max_RSS_num,CV_8UC1);

	// for(it_current_fingerprint = m_radio_fingerprints.begin(); it_current_fingerprint != m_radio_fingerprints.end();it_current_fingerprint ++)
	// {
	// 	vector<int> RSS_data_temp;
	// 	for(it_iterator_map = it_current_fingerprint->second.wifi_scan.begin(); 
	// 	 it_iterator_map != it_current_fingerprint->second.wifi_scan.end(); it_iterator_map++)
	// 	{
	// 		wifi_scan_temp[it_iterator_map->first].push_back(it_iterator_map->second);
	// 	}
	// 	sort(wifi_scan_temp.begin(),wifi_scan_temp.end());
		
	// 	// timestamp_cvmat[it_current_fingerprint->first] = wifi_scan_temp;
	// }
	
	// 构建 wifiRSS 与 0-256的映射	不需要 直接写进函数里
	// std::map<int,int> RSS_pixel;
	// for(int i = -29; i != -96 ; i--)
	// {
	// 	double pixel;
	// 	pixel = 3.82 * (i-(-96));
	// 	RSS_pixel[i] = (int)pixel;
	// }
	...
	for (long int current_time = min_timestamp; current_time <= max_timestamp; current_time = current_time + timeWindow)
	{
		...
		//std::map<std::string, int> wifi_scan;				 // first-mac地址 second-mac对应的RSS
		std::map<std::string, std::vector<int>> v_wifi_scan; // first-mac地址 这段时间内second-mac对应的RSS的数组
		std::map<std::string, std::vector<int>> v_wifi_scan_sorted;
		std::map<std::string, std::vector<int>> ::iterator wifi_scan_iterator;
		...
		for (iterator_search = m_radio_fingerprints.begin(); iterator_search != m_radio_fingerprints.end(); ++iterator_search) // 每次都遍历所有的WiFi指纹 根据timestamp来切割
		{
			...
			if (time_diff <= m_extend_time_threshold)
			{
				...
				for (it_scan = temp_wifi_scan.begin(); it_scan != temp_wifi_scan.end(); ++it_scan)
				{
					RSS_temp = it_scan->second;
					double pixel = 3.82 * (RSS_temp+96);
					v_wifi_scan[it_scan->first].push_back((int)pixel);
				}
				std::vector<int> RSS_data_temp;
				for(wifi_scan_iterator = v_wifi_scan.begin();wifi_scan_iterator != v_wifi_scan.end();++ wifi_scan_iterator)
				{
					RSS_data_temp = wifi_scan_iterator->second;
					sort(RSS_data_temp.begin(),RSS_data_temp.end());
					v_wifi_scan_sorted[wifi_scan_iterator->first] = RSS_data_temp;
				}
			}
		}
		m_radio_fingerprints_extended[timestamp].wifi_scan_extended = v_wifi_scan_sorted;
		// 把wifi 的所有mac 和对应的RSS 都存到wifi序列里面
		// //测试：
		// for(wifi_scan_iterator = v_wifi_scan_sorted.begin();wifi_scan_iterator!=v_wifi_scan_sorted.end();++wifi_scan_iterator)
		// {
		// 	cout<<"mac地址为" << wifi_scan_iterator->first<<"对应的横坐标为"
		// 	<< mac_graph[wifi_scan_iterator->first] << "下的 RSS 对应的像素值为：" ;
		// 	std::vector<int> temp_test_vector = wifi_scan_iterator->second;
		// 	std::vector<int>::iterator temp_         test_vector_iterator;
		// 	for(temp_test_vector_iterator=temp_test_vector.begin();temp_test_vector_iterator!=temp_test_vector.end();temp_test_vector_iterator++)
		// 	{
		// 		cout<< *temp_test_vector_iterator << " ";
		// 	}
		// 	cout<< endl;
		// }
		// cout << "这个序列结束" << endl;
		cout<< "max_RSS_num = " << max_RSS_num << endl;
		cv::Mat wifi_seq_pic = cv::Mat::zeros(mac_num,max_RSS_num,CV_8UC1);
		for(wifi_scan_iterator = v_wifi_scan_sorted.begin();wifi_scan_iterator!=v_wifi_scan_sorted.end();++wifi_scan_iterator)
		{
			int col_num = v_wifi_scan_sorted.size();
			int row_num = v_wifi_scan_sorted[wifi_scan_iterator->first].size();
			int col = mac_graph[wifi_scan_iterator->first];	//横坐标获取
			//for(int i = 0; i < col_num ; i++)
			//{
			std::vector<int> RSS_data_sorted_temp = v_wifi_scan_sorted[wifi_scan_iterator->first];
			for(int j = 0; j< row_num ; j ++)
			{
				int pixel = RSS_data_sorted_temp[j];
				wifi_seq_pic.at<uchar>(col,j) = pixel;
			}
			//}
		}
		// image.at<uchar>(j, i)
		std::string filename_wifi_graph = to_string(timestamp) + ".jpg";
		cv::imwrite(filename_wifi_graph,wifi_seq_pic);
	}
	// wifi 序列构建好了
