...
public:
	...
	int index_robot;	   // 这里是用于整合 里程计 和 WiFi指纹的 标记位 内容是m_robot_odometry中的first参数 也就是时间戳 表示本wifi_scan中最匹配的 RobotOdom 是时间戳为index_robot的RobotOdom
	int active_earfcn;	   // ###indicates the active earfcn in a scan
	int sensor_integrated; // ###是否有信息融合的标记位
	int gt_found;		   // 是否在m_robot_ground_truth中找到对应的groundtruth的标记位
};
...
std::map<long int, MobileScanning> m_radio_fingerprints; // recording the radio fingerprints 记录指纹 ###-first 为长整型（用来记录时间戳？ 以ms为单位）
														 // -second为 MobileScanning 类型（包括wifi_scan，frequency，xyz，累计距离，几个标记）
...
std::map<long int, MobileScanning> m_radio_fingerprints_optimized; // record the optimized results记录优化的WiFi指纹
...
class GridFingerprintRepresentation //指纹地图的表达 ：包括wifi_scan cell_scan x y z
{
public:
...
int m_odom_record; // number of odom nodes in the graph, anything higher this number will be regarded as the landmark 图优化中里程计的点的数量，高于这个数值的就会被视为地标
...
double m_sim_threshold = 0.7; //###最小的相似度阈值
...
// x y 坐标单位都是 m 范围是 x[-25~45] y[-20~130]
...
int main(int argc, char **argv)
{
	...
	// unique_cell_id对应的RSS如果满足条件 则 把m_lte_counter[unique_cell_id]下的标志位置为1 但是这个for好像没用
	...
	std::cout << "accumulated dist.." << accumulated_dist << std::endl;
	...
	std::cout << m_radio_fingerprints.size() << " " << m_radio_fingerprints_extended.size() << std::endl; //记录WiFi指纹文件里总共在多少个时间点进行过采样 以及生成的WiFi序列 在多少个时间点有

	std::cout << "Time Duration:" << (double)(max_timestamp - min_timestamp) / 1000.0 << std::endl; //采样这个WiFi指纹文件花了多久

	std::cout << "Assign GroundTruth.." << std::endl; //会不会是开始 为每个WiFi指纹文件分配 groundtruth
	...
	std::vector<int> detection;		 //###这是干嘛的	用来计算 miss_detection_likelihood
	std::vector<int> miss_detection; //###这是干嘛的	用来计算 miss_detection_likelihood
	...
	// 针对每个RSS值 在detection 和miss_detection 中 计算满足以binning_size+-2为范围的个数 分别得到num_detections和num_miss_detections
	...
	for (double b = 1; b >= 0.0; b = b - look_up_table)
	{
		...
		if (v_distance.size() > 0)
		{
			...
			m_similarty_variance_wifi.push_back(similarty_variance);				// 再把 相似度在min_sim max_sim之间的方差 存入m_similarty_variance_wifi中
...
Eigen::Matrix<double,6,6> information=Eigen::Matrix<double,6,6>::Identity();
	...
	for (it_current_fingerprint = m_radio_fingerprints_extended.begin(); it_current_fingerprint != m_radio_fingerprints_extended.end(); it_current_fingerprint++)
		for (it_ref_fingerprint = m_radio_fingerprints_extended.begin(); it_ref_fingerprint != m_radio_fingerprints_extended.end(); ++it_ref_fingerprint)
		{
			...
			// 两个WiFi序列都有效 且 WiFi序列1必须比WiFi序列2晚
			{
				double sim_lte = getCosineSimilarity(it_current_fingerprint->second.cell_scan, it_ref_fingerprint->second.cell_scan);
				...
				if (sim_lte >= lte_sim_thresh)
				{
					...
					lte_mean = m_similarty_variance_lte[similarity_table_index].mean;			// 把对应这对WiFi序列的相似度的 方差的平均值？记录
					lte_variance = m_similarty_variance_lte[similarity_table_index].variance;	// 把对应这对WiFi序列的相似度的 方差记录
				}
				...
				if (sim_wifi >= wifi_sim_thresh)
				{
					...
					wifi_mean = m_similarty_variance_wifi[similarity_table_index].mean;
					wifi_variance = m_similarty_variance_wifi[similarity_table_index].variance;
				}
				...
				if (sim_lte >= lte_sim_thresh && sim_wifi >= wifi_sim_thresh)
				{
					...
					double sum_variance = wifi_variance + lte_variance;
