...
static void publish_frontlaser_message( carmen_robot_laser_message laser_msg ) {
  IPC_RETURN_TYPE err;
  ...
  if( robot_front_laser.host != NULL ) {
    ...
    robot_front_laser.host = NULL;
  }
}

static void publish_rearlaser_message( carmen_robot_laser_message laser_msg ) {
  IPC_RETURN_TYPE err;
...
static int construct_laser_message( carmen_robot_laser_message *msg, double offset, int rear, double timestamp ) {
  int low;
  int high;
  double skew;
  double fraction;
  ...
  if( !rear ) {
    laser_ready = carmen_robot_get_skew(front_laser_count, &skew, CARMEN_ROBOT_FRONT_LASER_AVERAGE, msg->host);
    if( !laser_ready ) {
    ...
    laser_ready = carmen_robot_get_skew(rear_laser_count, &skew, CARMEN_ROBOT_REAR_LASER_AVERAGE, msg->host);
    if( !laser_ready ) {
  ...
  msg->robot_pose.x = carmen_robot_odometry[low].x
                      + fraction * (carmen_robot_odometry[high].x - carmen_robot_odometry[low].x);
  msg->robot_pose.y = carmen_robot_odometry[low].y
                      + fraction * (carmen_robot_odometry[high].y - carmen_robot_odometry[low].y);
  // AK: fixed interp bug ...
  msg->robot_pose.theta = carmen_robot_interpolate_heading(carmen_robot_odometry[low].theta,
                                                           carmen_robot_odometry[high].theta,
                                                           fraction);
  msg->robot_pose.theta = carmen_normalize_theta(msg->robot_pose.theta);

  msg->tv = carmen_robot_odometry[low].tv + fraction * (carmen_robot_odometry[high].tv - carmen_robot_odometry[low].tv);
  msg->rv = carmen_robot_odometry[low].rv + fraction * (carmen_robot_odometry[high].rv - carmen_robot_odometry[low].rv);

  if( rear )
    msg->laser_pose.theta = msg->robot_pose.theta + M_PI;
  else
    msg->laser_pose.theta = msg->robot_pose.theta;
  msg->laser_pose.theta = carmen_normalize_theta(msg->laser_pose.theta);
  msg->laser_pose.x = msg->robot_pose.x + offset * cos(msg->laser_pose.theta);
  msg->laser_pose.y = msg->robot_pose.y + offset * sin(msg->laser_pose.theta);
  return 1;
}
...
void carmen_robot_correct_laser_and_publish( void ) {
  ...
  if( front_laser_ready ) {
    if( construct_laser_message(&robot_front_laser, frontlaser_offset, 0, front_laser.timestamp) ) {
      fprintf(stderr, "f");
  ...
  if( rear_laser_ready ) {
    if( construct_laser_message(&robot_rear_laser, rearlaser_offset, 1, rear_laser.timestamp) ) {
      fprintf(stderr, "r");
...
static void check_message_data_chunk_sizes( carmen_laser_laser_message *laser_ptr ) {
  ...
  if( first ) {
    ...
    robot_laser.range = (float *)calloc(robot_laser.num_readings, sizeof(float));
    carmen_test_alloc(robot_laser.range);
    robot_laser.tooclose = (char *)calloc(robot_laser.num_readings, sizeof(char));
    carmen_test_alloc(robot_laser.tooclose);

    robot_laser.num_remissions = laser.num_remissions;

    if( robot_laser.num_remissions > 0 ) {
      robot_laser.remission = (float *)calloc(robot_laser.num_remissions, sizeof(float));
    ...
    robot_laser.num_readings = laser.num_readings;
    robot_laser.range = (float *)realloc(robot_laser.range, sizeof(float) * robot_laser.num_readings);
    carmen_test_alloc(robot_laser.range);
    robot_laser.tooclose = (char *)realloc(robot_laser.tooclose, sizeof(char) * robot_laser.num_readings);
    carmen_test_alloc(robot_laser.tooclose);

    robot_laser.num_remissions = laser.num_remissions;
    if( robot_laser.num_remissions > 0 ) {
      robot_laser.remission = (float *)realloc(robot_laser.remission, sizeof(float) * robot_laser.num_remissions);
      carmen_test_alloc(robot_laser.remission);
    } else
      robot_laser.remission = NULL;
...
static void laser_frontlaser_handler( void ) {
  int i;
  ...
  carmen_robot_update_skew(CARMEN_ROBOT_FRONT_LASER_AVERAGE, &front_laser_count, front_laser.timestamp, front_laser.host);

  memcpy(robot_front_laser.range, front_laser.range, robot_front_laser.num_readings * sizeof(float));
  memset(robot_front_laser.tooclose, 0, robot_front_laser.num_readings);
  if( robot_front_laser.num_remissions > 0 )
    memcpy(robot_front_laser.remission, front_laser.remission, robot_front_laser.num_remissions * sizeof(float));

  robot_front_laser.config = front_laser.config;

  carmen_robot_sensor_time_of_last_update = carmen_get_time();

  /** AK: 2 consecutive laser message updates without any odometry reading
   *      inbetween lead to a wrong timestamp of the laser data, since the
   *      laser data timestamp is not updated!
   *      FIXED: setting host field to NULL-pointer (which was necessary anyway)
   *             after publishing and freeing the memory, we can do a check on
   *             the host field whether message was already published or not.
   *             if not: also update the timestamp of the laser data!
   */
  if( robot_front_laser.host != NULL ) {
    robot_front_laser.timestamp = front_laser.timestamp;
  }

  if( carmen_robot_sensor_time_of_last_update - time_since_last_process < 1.0
                                                                          / carmen_robot_collision_avoidance_frequency )
    return;
  ...
  robot_front_laser.side_safety_dist = 0.5 * carmen_robot_config.width + carmen_robot_config.side_dist;
  ...
  for( i = 0; i < robot_front_laser.num_readings; i++, theta += delta_theta ) {
    ...
    carmen_geometry_move_pt_to_rotating_ref_frame(&obstacle_pt, carmen_robot_latest_odometry.tv,
...
static void laser_rearlaser_handler( void ) {
  int i;
  double safety_distance;
  double theta, delta_theta;
  carmen_traj_point_t robot_posn, obstacle_pt;
  ...
  carmen_robot_update_skew(CARMEN_ROBOT_REAR_LASER_AVERAGE, &rear_laser_count, rear_laser.timestamp, rear_laser.host);

  memcpy(robot_rear_laser.range, rear_laser.range, robot_rear_laser.num_readings * sizeof(float));
  memset(robot_rear_laser.tooclose, 0, robot_rear_laser.num_readings);
  if( robot_rear_laser.num_remissions > 0 )
    memcpy(robot_rear_laser.remission, rear_laser.remission, robot_rear_laser.num_remissions * sizeof(float));
  ...
  robot_rear_laser.side_safety_dist = carmen_robot_config.width / 2.0 + carmen_robot_config.side_dist;
...
void carmen_robot_add_laser_handlers( void ) {
  ...
  err = IPC_defineMsg(CARMEN_ROBOT_FRONTLASER_NAME, IPC_VARIABLE_LENGTH, CARMEN_ROBOT_FRONTLASER_FMT);
  carmen_test_ipc_exit(err, "Could not define", CARMEN_ROBOT_FRONTLASER_NAME);

  err = IPC_defineMsg(CARMEN_ROBOT_REARLASER_NAME, IPC_VARIABLE_LENGTH, CARMEN_ROBOT_REARLASER_FMT);
  carmen_test_ipc_exit(err, "Could not define", CARMEN_ROBOT_REARLASER_NAME);

  carmen_laser_subscribe_frontlaser_message(&front_laser, (carmen_handler_t)laser_frontlaser_handler,
                                            CARMEN_SUBSCRIBE_LATEST);
  carmen_laser_subscribe_rearlaser_message(&rear_laser, (carmen_handler_t)laser_rearlaser_handler,
                                           CARMEN_SUBSCRIBE_LATEST);
...
void carmen_robot_add_laser_parameters( int argc, char** argv ) {
  ...
  carmen_param_t param_list[] = { { "robot", "frontlaser_offset", CARMEN_PARAM_DOUBLE, &frontlaser_offset, 1, NULL }, {
...
/* AK: updated from carmen 0.7.4 to use laser offsets!
 void
