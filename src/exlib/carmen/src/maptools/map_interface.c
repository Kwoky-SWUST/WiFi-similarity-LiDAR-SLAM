...
carmen_map_get_gridmap_by_name(char *name, carmen_map_p client_map)
{
  ...
  if (client_map)
	...
	client_map->map[i] = client_map->complete_map +
      ...
      i = carmen_map_get_global_offset( &(client_map->offset) );
      if ( i != 0 )
      {
        carmen_warn( "carmen_map_get_global_offset returned %d.\n", i );
        exit(-1);
      }
    }
...
{
  // Problem hier: localizecore.c verwendet nicht diese Umrechnungen!
/*  double x = carmen_map_point->x * carmen_map_point->map->config.resolution + carmen_map_point->map->offset.x;
  double y = carmen_map_point->y * carmen_map_point->map->config.resolution + carmen_map_point->map->offset.y;*/
...
{
  // Problem hier: localizecore.c verwendet nicht diese Umrechnungen!
/*  int x = carmen_round((world_point->pose.x - world_point->map->offset.x) / world_point->map->config.resolution);
  int y = carmen_round((world_point->pose.y - world_point->map->offset.y) / world_point->map->config.resolution);*/
  int x = carmen_round(world_point->pose.x / world_point->map->config.resolution);
  int y = carmen_round(world_point->pose.y / world_point->map->config.resolution);
