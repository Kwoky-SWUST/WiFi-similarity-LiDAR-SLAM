...
static void draw_sonar_arcs(GdkPixmap *pixmap)
{
  ...
  printf( "sonar.num_sonars: %d\n", sonar.num_sonars );
  for(i=0; i<sonar.num_sonars; i++)
    {
      printf( "  sonar.ranges[%d] = %8.2f\n", i, sonar.ranges[i] );
      printf( "  sonar.positions[%d].x = %8.2f\n", i, sonar.positions[i].x );
	...
	radius;
      w=radius*2;
      h=radius*2;
      theta1=carmen_radians_to_degrees(sonar.positions[i].theta-sonar.sensor_angle/2)*64;
      theta2=carmen_radians_to_degrees(sonar.sensor_angle)*64;
      gdk_draw_arc(pixmap,drawing_gc,TRUE,x,y,w,h,theta1,theta2);
    }
}
