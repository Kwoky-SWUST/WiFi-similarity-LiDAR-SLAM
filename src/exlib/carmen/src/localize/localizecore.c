...
static void priority_queue_add( priority_queue_p queue, carmen_point_t point, float prob )
{
  ...
  if( queue->num_elements == 0 ) {
    ...
    queue->last = temp;
    queue->num_elements++;
  } else if( prob > queue->last->prob || queue->num_elements < queue->max_elements ) {
    mark = queue->last;
...
static void initialize_temp_weights( carmen_localize_particle_filter_p filter )
{
  ...
  filter->temp_weights = (float **)calloc(filter->param->num_particles, sizeof(float *));
...
static void realloc_temp_weights( carmen_localize_particle_filter_p filter, int num_particles )
{
  int i;

  for( i = 0; i < filter->param->num_particles; i++ )
    free(filter->temp_weights[i]);
  filter->temp_weights = (float **)realloc(filter->temp_weights, num_particles * sizeof(float *));
  carmen_test_alloc(filter->temp_weights);
...
carmen_localize_particle_filter_p carmen_localize_particle_filter_new( carmen_localize_param_p param )
{
  ...
  filter = (carmen_localize_particle_filter_p)calloc(1, sizeof(carmen_localize_particle_filter_t));
  ...
  filter->particles = (carmen_localize_particle_p)calloc(filter->param->num_particles, sizeof(carmen_localize_particle_t));
...
void carmen_localize_initialize_particles_uniform( carmen_localize_particle_filter_p filter, carmen_robot_laser_message *laser,
                                                   carmen_localize_map_p map )
{
  ...
  for( i = 0; i < laser->num_readings; i++ ) {
    if( laser->range[i] < laser->config.maximum_range && laser->range[i] < filter->param->max_range )
      beam_valid[i] = 1;
    else
      beam_valid[i] = 0;
  }
  ...
  for( i = 0; i < laser->num_readings; i++ ) {
    angle = laser->config.start_angle + i * laser->config.angular_resolution;

    laser_x[i] = (filter->param->front_laser_offset + laser->range[i] * cos(angle)) / map->config.resolution;
    laser_y[i] = (laser->range[i] * sin(angle)) / map->config.resolution;
  }

  for( i = 0; i < filter->param->global_test_samples; i++ ) {
    if( i % 10000 == 0 ) {
      fprintf(stderr, "\rDoing global localization... (%.1f%% complete)", i / (float)filter->param->global_test_samples * 100.0);
    ...
    } while( map->carmen_map.map[(int)point.x][(int)point.y] > filter->param->occupied_prob || map->carmen_map.map[(int)point.x][(int)point.y] == -1 );
    point.theta = carmen_uniform_random(-M_PI, M_PI);

    prob = 0.0;
    ctheta = cos(point.theta);
    stheta = sin(point.theta);
    for( j = 0; j < laser->num_readings && (queue->last == NULL || prob > queue->last->prob); j += filter->param->laser_skip ) {

      if( beam_valid[j] ) {
        x_l = point.x + laser_x[j] * ctheta - laser_y[j] * stheta;
        y_l = point.y + laser_x[j] * stheta + laser_y[j] * ctheta;

        if( x_l >= 0 && y_l >= 0 && x_l < map->config.x_size && y_l < map->config.y_size )
  ...
  if( filter->param->do_scanmatching ) {
    for( i = 0; i < filter->param->num_particles; i++ ) {
      ...
      carmen_localize_laser_scan_gd(laser->num_readings, laser->range, laser->config.angular_resolution, laser->config.start_angle, &point,
                                    filter->param->front_laser_offset, map, filter->param->laser_skip);
...
void carmen_localize_initialize_particles_gaussians( carmen_localize_particle_filter_p filter, int num_modes, carmen_point_t *mean,
                                                     carmen_point_t *std )
{
...
int carmen_localize_initialize_particles_placename( carmen_localize_particle_filter_p filter, carmen_map_placelist_p placelist,
                                                    char *placename )
{
  ...
  for( i = 0; i < placelist->num_places; i++ )
    if( strcmp(placename, placelist->places[i].name) == 0 )
      break;
  if( i == placelist->num_places || placelist->places[i].type != CARMEN_LOCALIZATION_INIT_TYPE )
    return -1;
...
void carmen_localize_initialize_particles_manual( carmen_localize_particle_filter_p filter, double *x, double *y, double *theta,
                                                  double *weight, int num_particles )
{
  int i;

  if( num_particles != filter->param->num_particles ) {
    filter->particles = (carmen_localize_particle_p)realloc(filter->particles, num_particles * sizeof(carmen_localize_particle_t));
    carmen_test_alloc(filter->particles);
...
void carmen_localize_incorporate_odometry( carmen_localize_particle_filter_p filter, carmen_point_t odometry_position )
{
  ...
  delta_theta = carmen_normalize_theta(odometry_position.theta - filter->last_odometry_position.theta);
  odom_theta = atan2(dy, dx);
  backwards = (dx * cos(odometry_position.theta) + dy * sin(odometry_position.theta) < 0);

  filter->distance_travelled += delta_t;

#ifndef OLD_MOTION_MODEL
  for( i = 0; i < filter->param->num_particles; i++ ) {
    downrange = carmen_localize_sample_noisy_downrange(delta_t, delta_theta, filter->param->motion_model);
    crossrange = carmen_localize_sample_noisy_crossrange(delta_t, delta_theta, filter->param->motion_model);
    turn = carmen_localize_sample_noisy_turn(delta_t, delta_theta, filter->param->motion_model);

    if( backwards ) {
      filter->particles[i].x -= downrange * cos(filter->particles[i].theta + turn / 2.0) + crossrange * cos(
          filter->particles[i].theta + turn / 2.0 + M_PI / 2.0);
      filter->particles[i].y -= downrange * sin(filter->particles[i].theta + turn / 2.0) + crossrange * sin(
          filter->particles[i].theta + turn / 2.0 + M_PI / 2.0);
    } else {
      filter->particles[i].x += downrange * cos(filter->particles[i].theta + turn / 2.0) + crossrange * cos(
          filter->particles[i].theta + turn / 2.0 + M_PI / 2.0);
...
static int global_mode_test( carmen_localize_particle_filter_p filter )
{
  ...
  for( i = 0; i < filter->param->num_particles; i++ )
    if( fabs(filter->particles[i].x - mean_x) > filter->param->global_distance_threshold || fabs(filter->particles[i].y - mean_y) > filter->param->global_distance_threshold )
      return 1;
  return 0;
}

/* incorporate a single laser scan into the paritcle filter */
/* TODO (AK): the observation model for a laser scan is not close to state of the art ...
 */
void carmen_localize_incorporate_laser( carmen_localize_particle_filter_p filter, carmen_localize_map_p map, int num_readings, float *range,
                                        double forward_offset, double angular_resolution, double laser_maxrange, double first_beam_angle,
                                        int backwards )
{
  ...
  /* TODO (AK): this weight reset is very fishy!!!
   * if the update_distance is set to some high value, particles don't get resampled, which means
   * that this is actually not a filter in the way it is supposed to be!
   * if the weights are reset to uniform before the update step, all information on previous
   * evidence in the form of laser readings is just discarded -> adios bayesian filtering ...
   * on the other hand, if the update_distance is set to some low value, the pf may degenerate
   * very fast, since one noisy reading would lead to the convergence to a wrong pose in the map,
   * which may not be recovered any more ...
   */
  ...
  for( i = 0; i < num_readings; i++ ) {
    ...
    laser_x[i] = (forward_offset + range[i] * cos(angle)) / map->config.resolution;
    laser_y[i] = (range[i] * sin(angle)) / map->config.resolution;
    if( backwards ) {
      laser_x[i] = -laser_x[i];
      laser_y[i] = -laser_y[i];
    }
    if( (i % filter->param->laser_skip) == 0 && range[i] < filter->param->max_range && range[i] < laser_maxrange )
    ...
    for( i = 0; i < filter->param->num_particles; i++ ) {
      ...
      for( j = 0; j < num_readings; j += filter->param->laser_skip )
        if( filter->laser_mask[j] ) {
          ...
          else if( filter->param->constrain_to_map && (robot_x < 0 || robot_y < 0 || robot_x >= map->config.x_size || robot_y >= map->config.y_size || map->carmen_map.map[robot_x][robot_y] > filter->param->occupied_prob) )
            filter->particles[i].weight += global_log_small_prob;
          else
            filter->particles[i].weight += map->gprob[x][y] * filter->param->global_evidence_weight;
        }
    }
  } else {
...
//            const double weight = map->prob[x][y] * filter->param->global_evidence_weight;
//            filter->temp_weights[i][j] = (weight > log_small_prob) ? weight : log_small_prob;
...
//  fflush(stdout);
  ...
  temp_particles = (carmen_localize_particle_p)calloc(filter->param->num_particles, sizeof(carmen_localize_particle_t));
  ...
  for( i = 0; i < filter->param->num_particles; i++ ) {
    ...
    while( position > cumulative_sum[which_particle] )
      which_particle++;
    memcpy(temp_particles + i, filter->particles + which_particle, sizeof(carmen_localize_particle_t));
  }
...
void carmen_localize_run( carmen_localize_particle_filter_p filter, carmen_localize_map_p map, carmen_robot_laser_message *laser,
                          double forward_offset, int backwards )
{
  ...
  if( filter->param->use_sensor ) {
    ...
    carmen_localize_incorporate_laser(filter, map, laser->num_readings, laser->range, forward_offset, laser->config.angular_resolution,
                                      laser->config.maximum_range, laser->config.start_angle, backwards);
...
void carmen_localize_laser_scan_gd( int num_readings, float *range, double angular_resolution, double first_beam_angle,
                                    carmen_point_p laser_pos, double forward_offset, carmen_localize_map_p map, int laser_skip )
{
  ...
  do {
    ...
    for( i = 0; i < num_readings; i += laser_skip ) {
      ...
      x_l = (int) ( (laser_pos->x + forward_offset * cos(laser_pos->theta) + range_x) / map->config.resolution);
      y_l = (int) ( (laser_pos->y + forward_offset * sin(laser_pos->theta) + range_y) / map->config.resolution);

      if( x_l >= 0 && y_l >= 0 && x_l < map->config.x_size && y_l < map->config.y_size ) {
        grad_x += map->x_offset[x_l][y_l];
        grad_y += map->y_offset[x_l][y_l];
        grad_theta += range_x * map->y_offset[x_l][y_l] - range_y * map->x_offset[x_l][y_l];
      }
    }
    ...
    count++;
  } while( count < 20 && (grad_x > 0.05 || grad_y < 0.05 || grad_theta < 0.25 * 180.0 / M_PI) );
}

void carmen_localize_summarize( carmen_localize_particle_filter_p filter, carmen_localize_summary_p summary, carmen_localize_map_p map,
                                int num_readings, float *range, double angular_resolution, double first_beam_angle, double forward_offset,
                                int backwards )
{
  ...
  for( i = 0; i < filter->param->num_particles; i++ ) {
    ...
    diff_theta = carmen_normalize_theta(filter->particles[i].theta - summary->mean.theta);
  ...
  if( filter->param->do_scanmatching )
    carmen_localize_laser_scan_gd(summary->num_readings, range, angular_resolution, first_beam_angle, &summary->mean, forward_offset, map,
                                  1);
  ...
  for( i = 0; i < num_readings; i++ ) {
    ...
    if( backwards ) {
      angle = summary->mean.theta + M_PI + first_beam_angle + i * angular_resolution;
      summary->mean_scan[i].x = summary->mean.x - forward_offset * cos(summary->mean.theta) + cos(angle) * range[i];
      summary->mean_scan[i].y = summary->mean.y - forward_offset * sin(summary->mean.theta) + sin(angle) * range[i];
    } else {
      angle = summary->mean.theta + first_beam_angle + i * angular_resolution;
      summary->mean_scan[i].x = summary->mean.x + forward_offset * cos(summary->mean.theta) + cos(angle) * range[i];
      summary->mean_scan[i].y = summary->mean.y + forward_offset * sin(summary->mean.theta) + sin(angle) * range[i];
    }
