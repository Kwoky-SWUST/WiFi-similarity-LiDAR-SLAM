...
carmen_localize_initialize_message init_message;
int desired_number_of_particles;
...
void publish_particles( carmen_localize_particle_filter_p filter, carmen_localize_summary_p summary )
{
...
void publish_sensor( carmen_localize_particle_filter_p filter, carmen_localize_summary_p summary,
                     carmen_robot_laser_message *laser, int front )
{
  ...
  carmen_test_ipc_exit(err, "Could not publish", CARMEN_LOCALIZE_SENSOR_NAME);
}
...
void carmen_localize_initialize_handler( carmen_localize_initialize_message *initialize_msg )
{
  if( initialize_msg->distribution == CARMEN_INITIALIZE_GAUSSIAN )
    carmen_localize_initialize_particles_gaussians(filter, initialize_msg->num_modes, initialize_msg->mean,
                                                   initialize_msg->std);
...
{
  carmen_localize_initialize_particles_placename(filter, &placelist, init_place->placename);
  publish_particles(filter, &summary);
}
...
{
  fprintf(stderr, "F");
  carmen_localize_run(filter, &map, flaser, filter->param->front_laser_offset, 0);
  if( filter->initialized ) {
    carmen_localize_summarize(filter, &summary, &map, flaser->num_readings, flaser->range,
                              filter->param->front_laser_offset, flaser->config.angular_resolution,
...
static void globalpos_query_handler( MSG_INSTANCE msgRef, BYTE_ARRAY callData,
...
static void map_query_handler( MSG_INSTANCE msgRef, BYTE_ARRAY callData, void *clientData __attribute__ ((unused)) )
{
  FORMATTER_PTR formatter;
  IPC_RETURN_TYPE err;
  static carmen_localize_map_query_message msg;
  static carmen_localize_map_message response;
  ...
  formatter = IPC_msgInstanceFormatter(msgRef);
  err = IPC_unmarshallData(formatter, callData, &msg, sizeof(carmen_localize_map_query_message));
  IPC_freeByteArray(callData);
  ...
  compress_return = compress((void *)compressed_map, (uLong *)&compress_buf_size, (void *)response.data,
                             (uLong)response.size);
...
/* handler for parameter changes of the likelihood map */

static void lmap_param_update_handler()
{
  carmen_map_p pCurMap = carmen_map_copy( &map.carmen_map );
  map_update_handler( pCurMap );
//  map_update_handler( &map.carmen_map );
}


static void num_particles_update_handler()
{
  // reallocate memory
  if( desired_number_of_particles != filter->param->num_particles ) {
    filter->particles = (carmen_localize_particle_p)realloc(filter->particles, desired_number_of_particles * sizeof(carmen_localize_particle_t));
    carmen_test_alloc(filter->particles);

    int i;
    for( i = 0; i < filter->param->num_particles; i++ )
      free(filter->temp_weights[i]);
    filter->temp_weights = (float **)realloc(filter->temp_weights, desired_number_of_particles * sizeof(float *));
    carmen_test_alloc(filter->temp_weights);
    for( i = 0; i < desired_number_of_particles; i++ ) {
      filter->temp_weights[i] = (float *)calloc(MAX_BEAMS_PER_SCAN, sizeof(float));
      carmen_test_alloc(filter->temp_weights[i]);
    }

    filter->param->num_particles = desired_number_of_particles;
  }

  // initialize particles
  carmen_localize_initialize_particles_gaussians(filter, init_message.num_modes, &summary.mean, &summary.std);
}
...
int register_ipc_messages( void )
{
  ...
  err = IPC_defineMsg(CARMEN_LOCALIZE_INITIALIZE_NAME, IPC_VARIABLE_LENGTH, CARMEN_LOCALIZE_INITIALIZE_FMT);
  carmen_test_ipc_exit(err, "Could not define", CARMEN_LOCALIZE_INITIALIZE_NAME);

  /* subscribe to initialization messages */
  carmen_localize_subscribe_initialize_message(&init_message, (carmen_handler_t)carmen_localize_initialize_handler,
                                               CARMEN_SUBSCRIBE_LATEST);

  carmen_localize_subscribe_initialize_placename_message(NULL,
                                                         (carmen_handler_t)carmen_localize_initialize_placename_handler,
                                                         CARMEN_SUBSCRIBE_LATEST);

  /* register map request message */
  err = IPC_defineMsg(CARMEN_LOCALIZE_MAP_QUERY_NAME, IPC_VARIABLE_LENGTH, CARMEN_LOCALIZE_MAP_QUERY_FMT);
  carmen_test_ipc_exit(err, "Could not define", CARMEN_LOCALIZE_MAP_QUERY_NAME);

  err = IPC_defineMsg(CARMEN_LOCALIZE_MAP_NAME, IPC_VARIABLE_LENGTH, CARMEN_LOCALIZE_MAP_FMT);
  ...
  err = IPC_defineMsg(CARMEN_LOCALIZE_GLOBALPOS_QUERY_NAME, IPC_VARIABLE_LENGTH, CARMEN_DEFAULT_MESSAGE_FMT);
  carmen_test_ipc_exit(err, "Could not define", CARMEN_LOCALIZE_MAP_QUERY_NAME);

  /* subscribe to globalpos request message */
  err = IPC_subscribe(CARMEN_LOCALIZE_GLOBALPOS_QUERY_NAME, globalpos_query_handler, NULL);
  carmen_test_ipc(err, "Could not subscribe", CARMEN_LOCALIZE_GLOBALPOS_QUERY_NAME);
...
{
  carmen_param_t param_list[] = { { "robot",
                                    "frontlaser_offset",
                                    CARMEN_PARAM_DOUBLE,
                                  ...
                                  { "robot",
                                    "rearlaser_offset",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->rear_laser_offset,
                                    0,
                                    NULL },
                                  { "localize",
                                    "use_rear_laser",
                                    CARMEN_PARAM_ONOFF,
                                    &param->use_rear_laser,
                                    0,
                                    NULL },
                                  { "localize",
                                    "num_particles",
                                    CARMEN_PARAM_INT,
                                    &desired_number_of_particles,
                                    1,
                                    (carmen_handler_t)num_particles_update_handler },
                                  { "localize",
                                    "laser_max_range",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->max_range,
                                  ...
                                  { "localize",
                                    "update_distance",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->update_distance,
                                    1,
                                    NULL },
                                  { "localize",
                                    "integrate_angle",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->integrate_angle,
                                    0,
                                    NULL },
                                  { "localize",
                                    "do_scanmatching",
                                    CARMEN_PARAM_ONOFF,
                                    &param->do_scanmatching,
                                    1,
                                    NULL },
                                  { "localize",
                                    "constrain_to_map",
                                    CARMEN_PARAM_ONOFF,
                                    &param->constrain_to_map,
                                    1,
                                    NULL },
                                  ...
                                  { "localize",
                                    ...
                                    &param->occupied_prob,
                                    1,
                                    (carmen_handler_t)lmap_param_update_handler },
                                  { "localize",
                                    "lmap_std",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->lmap_std,
                                  ...
                                  { "localize",
                                    "global_lmap_std",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->global_lmap_std,
                                    1,
                                    (carmen_handler_t)lmap_param_update_handler },
                                  { "localize",
                                    "global_evidence_weight",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->global_evidence_weight,
                                    1,
                                    NULL },
                                  { "localize",
                                    "global_distance_threshold",
                                    CARMEN_PARAM_DOUBLE,
                                    &param->global_distance_threshold,
                                    1,
                                    NULL },
                                  { "localize",
                                    "global_test_samples",
                                    CARMEN_PARAM_INT,
                                    &param->global_test_samples,
                                    1,
                                    NULL },
                                  { "localize",
                                    "use_sensor",
  ...
  param->num_particles = desired_number_of_particles;
}
...
int main( int argc, char **argv )
{
  ...
  carmen_robot_subscribe_frontlaser_message(&front_laser, (carmen_handler_t)robot_frontlaser_handler,
                                            CARMEN_SUBSCRIBE_LATEST);
  ...
  carmen_map_subscribe_gridmap_update_message(NULL, (carmen_handler_t)map_update_handler, CARMEN_SUBSCRIBE_LATEST);
