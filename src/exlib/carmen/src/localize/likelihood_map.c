...
static void create_distance_map( carmen_map_p cmap, carmen_localize_map_p lmap, carmen_localize_param_p param )
{
  int x, y, i, j, border;
  double v;
...
static void create_likelihood_map( carmen_localize_map_p lmap, float **prob, float std )
{
  int x, y;
  double p, max;
  ...
  for( x = 0; x < lmap->config.x_size; x++ )
    for( y = 0; y < lmap->config.y_size; y++ ) {
      p = exp(-0.5 * carmen_square(lmap->distance[x][y] * lmap->config.resolution / std));
      if( p > max )
        max = p;
      prob[x][y] = p;
    }
...
void carmen_to_localize_map( carmen_map_p cmap, carmen_localize_map_p lmap, carmen_localize_param_p param )
{
  int i;
  ...
  lmap->complete_distance = (float *)calloc(lmap->config.x_size * lmap->config.y_size, sizeof(float));
  ...
  /* allocate prob map */
  lmap->complete_prob = (float *)calloc(lmap->config.x_size * lmap->config.y_size, sizeof(float));
...
//    lmap->prob[i] = lmap->complete_prob + i * lmap->config.y_size;
  ...
  /* allocate gprob map */
  lmap->complete_gprob = (float *)calloc(lmap->config.x_size * lmap->config.y_size, sizeof(float));
  ...
  lmap->complete_x_offset = (short int *)calloc(lmap->config.x_size * lmap->config.y_size, sizeof(short int));
  carmen_test_alloc(lmap->complete_x_offset);
  lmap->x_offset = (short int **)calloc(lmap->config.x_size, sizeof(short int *));
  carmen_test_alloc(lmap->x_offset);
  for( i = 0; i < lmap->config.x_size; i++ )
    lmap->x_offset[i] = lmap->complete_x_offset + i * lmap->config.y_size;

  /* allocate y offset map */
  lmap->complete_y_offset = (short int *)calloc(lmap->config.x_size * lmap->config.y_size, sizeof(short int));
  carmen_test_alloc(lmap->complete_y_offset);
  lmap->y_offset = (short int **)calloc(lmap->config.x_size, sizeof(short int *));
...
void carmen_localize_write_map_to_ppm( char *filename, carmen_localize_map_p map )
{
  FILE *fp;
  int i, j;
  double max;
  ...
  for( i = map->config.y_size - 1; i >= 0; i-- )
    for( j = 0; j < map->config.x_size; j++ )
      if( map->carmen_map.map[j][i] == -1 ) {
        ...
        fprintf(fp, "%c", 255 - (int) (map->carmen_map.map[j][i] / max * 255.0));
        fprintf(fp, "%c", 255 - (int) (map->carmen_map.map[j][i] / max * 255.0));
        fprintf(fp, "%c", 255 - (int) (map->carmen_map.map[j][i] / max * 255.0));
      }
  fclose(fp);
...
void carmen_localize_write_distance_map_to_ppm( char *filename, carmen_localize_map_p map )
{
  FILE *fp;
  int i, j;
  double max;
...
void carmen_localize_write_likelihood_map_to_ppm( char *filename, carmen_localize_map_p map, float **prob )
{
  FILE *fp;
  int i, j;
  double p, max;
