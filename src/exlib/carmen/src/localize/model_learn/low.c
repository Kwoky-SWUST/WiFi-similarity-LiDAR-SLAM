...
inline double QuickScore( TSense sense, int index, int sampleNum )
{
  ...
  eval = LowLineTrace(
      (int) (newSample[sampleNum].x + (cos(sense[index].theta + newSample[sampleNum].theta) * distance)),
      ...
      3.5,
      l_particle[newSample[sampleNum].parent].ancestryNode->ID, 3);
...
void Localize( TSense sense )
{
  ...
  distance = sqrt( ( (odometry.x - lastX) * (odometry.x - lastX))
                  + ( (odometry.y - lastY) * (odometry.y - lastY)))
             * MAP_SCALE;
...
{
  printf( "Destroying ancestry tree ... " );
  fflush(stdout);
  ...
  for( i = 0; i < ID_NUMBER - 1; i++ ) {
    ...
    if( (particleID[i].ID == i) && (particleID[i].parent != NULL) && (particleID[i].parent->numChildren == 1) ) {
      ...
      if( parentNode->size < (parentNode->total + particleID[i].total) ) {
        ...
        if( workArray == NULL )
          fprintf(stderr, "Malloc failed for workArray\n");
...
//
int ReadLog( carmen_FILE *logFile, carmen_logfile_index_p logfile_index, TSense sense )
{
  int i, max;
  char line[65536];
  int laser;
...
#if 1 // AK fix: use robot pose, since the robot center is assumed as rotation center and an offset is applied through TURN_RADIUS (see ThisRobot.h)
  // Read x and y coordinates.
  odometry.x = laser_msg.robot_pose.x;
  odometry.y = laser_msg.robot_pose.y;
  // Read the facing angle of the robot.
  odometry.theta = laser_msg.robot_pose.theta;
...
void PrintMap( char *name, TAncestor *parent, int particles, double overlayX, double overlayY, double overlayTheta )
{
  ...
  if( particles )
    for( i = 0; i < l_cur_particles_used; i++ )
      if( (l_particle[i].x > 0) && (l_particle[i].x < MAP_WIDTH) && (l_particle[i].y > 0)
          && (l_particle[i].y < MAP_HEIGHT) )
...
void LowSlam( TPath **path, TSenseLog **obs )
{
  ...
  while( curGeneration < LEARN_DURATION ) {
    ...
    if( overflow > 0 ) {
      ...
      if( tempObs->next == NULL )
        fprintf(stderr, "Malloc failed in making a new observation!\n");
      for( i = 0; i < SENSE_NUMBER; i++ ) {
