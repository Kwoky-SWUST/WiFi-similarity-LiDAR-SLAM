...
{
	static carmen_pantilt_status_message status;
	...
	//get current pan&tilt values
	char buffer[BUFFER_SIZE];
	PantiltPosType currentPos;
	writeCMD( pDevice, "PP", buffer );
	sscanf(buffer, "Current Pan position is %d", &currentPos.pan);
	writeCMD( pDevice, "TP", buffer );
	sscanf(buffer, "Current Tilt position is %d", &currentPos.tilt);

	status.pan = pticks2rad( currentPos.pan );
	status.tilt = tticks2rad( currentPos.tilt );

	err = IPC_publishData (CARMEN_PANTILT_STATUS_MESSAGE_NAME, &status );
	carmen_test_ipc(err, "Could not publish", CARMEN_PANTILT_STATUS_MESSAGE_NAME);
}
