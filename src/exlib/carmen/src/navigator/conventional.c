...
/** convert occupancy grid map into a cost map **/
                                     ...
                                     carmen_navigator_config_t *navigator_conf)
{
  ...
  // AK: force update of the whole map - changes in the window actually may influence the costmap outside
  // the window; vice-versa, effects of the static map outside the window do (currently) not spread to the
  // local map as they should: still if the window size is big enough, the effect may be negligible since
  // the local neighborhood gets updated frequently which should be sufficient for the local planning
  if ( FALSE &&
      robot_posn && navigator_conf) {

    int window_width = (int)(navigator_conf->map_update_radius / robot_posn->map->config.resolution + 0.5);
    x_start = robot_posn->x - window_width;
    x_start = carmen_clamp(0, x_start, x_size);
    y_start = robot_posn->y - window_width;
    y_start = carmen_clamp(0, y_start, y_size);
    x_end = robot_posn->x + window_width + 1;
    x_end = carmen_clamp(0, x_end, x_size);
    y_end = robot_posn->y + window_width + 1;
...
//    printf( "X[%d:%d:%d] -- Y[%d:%d:%d]\n", x_start - robot_posn->x, robot_posn->x, x_end - robot_posn->x,
//                                            y_start - robot_posn->y, robot_posn->y, y_end - robot_posn->y);
//  }
  ...
  for (x_index = x_start; x_index < x_end; x_index++) {
    if (x_index < 1 || x_index >= x_size-1 ) continue;
    cost_ptr = costs+x_index*y_size+y_start;
    for (y_index = y_start; y_index < y_end; y_index++, cost_ptr++) {
      if( y_index < 1 || y_index >= y_size-1 ) continue;
  ...
  // set the cost at the current robot position to MIN_COST
  if (robot_posn != NULL) {
    /* AK: the robot position is already in the map coordinate frame (and in grid cell coordinates)
     *     so we don't need the transform to the grid!
     */

//    x_index = robot_posn->x / carmen_planner_map->config.resolution;
//    y_index = robot_posn->y / carmen_planner_map->config.resolution;

    x_index = robot_posn->x;
    y_index = robot_posn->y;
...
carmen_conventional_dynamic_program(int goal_x, int goal_y)
{
  ...
  if ((int)(cur_time - last_print) > 10) {
    carmen_verbose("Time since last DP: %d secs, %d usecs\n", (int)(cur_time - last_time), ((int)((cur_time-last_time)*1e6)) % 1000000);
    last_print = cur_time;
  }
  last_time = cur_time;
...
/* take the neighbor with max utility value as next waypoint */
void
