...
typedef enum {NO_PLACEMENT, PLACING_ROBOT, ORIENTING_ROBOT,
  ...
  static GtkToggleActionEntry toggle_entries[] = {
                                                  ...
                                                  {"BlackWhite", NULL, "Black&White", NULL, NULL, G_CALLBACK(switch_localize_display), FALSE},
  ...
  static GtkRadioActionEntry radio_entries[] = {
                                                ...
                                                {"PlannerMap", NULL, "_PlannerMap", NULL, NULL, CARMEN_PLANNER_MAP_v},
  ...
  navigator_graphics_reset(void)
  {
    ...
    assign_variable("/ui/MainMenu/DisplayMenu/TrackRobot", -1, DEFAULT_TRACK_ROBOT);
    assign_variable("/ui/MainMenu/DisplayMenu/DrawWaypoints", -1, DEFAULT_DRAW_WAYPOINTS);
    assign_variable("/ui/MainMenu/DisplayMenu/ShowParticles", -1, DEFAULT_SHOW_PARTICLES);
    assign_variable("/ui/MainMenu/DisplayMenu/ShowGaussians", -1, DEFAULT_SHOW_GAUSSIANS);
    assign_variable("/ui/MainMenu/DisplayMenu/ShowLaserData", -1, DEFAULT_SHOW_LASER);
    assign_variable("/ui/MainMenu/SimulatorMenu/SimShowTruePosition", -1, DEFAULT_SHOW_SIMULATOR);
    assign_variable("/ui/MainMenu/SimulatorMenu/SimShowObjects", -1, DEFAULT_SHOW_TRACKED_OBJECTS);
  }
  ...
  static void switch_display(GtkAction *action,
                             gpointer user_data __attribute__ ((unused)))
  {
    ...
    if ( display == new_display )
       // && display != CARMEN_PLANNER_MAP_v) // AK: plot the intermediate map
      return;
  ...
  static void draw_lasers(GtkMapViewer *the_map_view, double pixel_size)
  {
    ...
    for(index = 0; index < sensor_msg.num_readings;
      ...
      angle = sensor_msg.pose.theta + sensor_msg.config.start_angle  +
          index * sensor_msg.config.angular_resolution;
  ...
  static void draw_simulated_robot(GtkMapViewer *the_map_view, double pixel_size)
  {
    ...
    draw_robot_shape(the_map_view, &simulator_trueposition, TRUE, &carmen_blue, pixel_size);
    draw_robot_shape(the_map_view, &simulator_trueposition, FALSE, &carmen_black, pixel_size);
  ...
  static void draw_robot(GtkMapViewer *the_map_view, double pixel_size)
  {
    ...
    robot_radius = robot;
    robot_radius.pose.x = robot_radius.pose.x + cos(robot_radius.pose.theta)*0.2;
    robot_radius.pose.y = robot_radius.pose.y + sin(robot_radius.pose.theta)*0.2;

    carmen_map_graphics_draw_line(the_map_view, &carmen_black, &robot, &robot_radius);
  ...
  void draw_robot_objects(GtkMapViewer *the_map_view)
  {
    ...
    for (index = 1; index < num_path_points; index++) {
      ...
      if (nav_panel_config->draw_waypoints) {
        ...
        carmen_map_graphics_draw_line(the_map_view, &path_colour, &path_x_1, &path_x_2);
    ...
    if (goal.pose.x > 0 && goal.pose.y > 0 && goal.map != NULL) {
      ...
      carmen_map_graphics_draw_circle(the_map_view, &goal_colour, TRUE, &goal, goal_size);
      carmen_map_graphics_draw_circle(the_map_view, &carmen_black, FALSE, &goal, goal_size);
    }
    ...
    if (nav_panel_config->show_simulator_objects) {
      ...
      if (simulator_objects) {
        for (index = 0; index < simulator_objects->length; index++) {
          ...
          if (black_and_white)
            carmen_map_graphics_draw_circle(the_map_view, &carmen_grey, TRUE, &particle, circle_size);
          else
    ...
    if (black_and_white) {
      carmen_map_graphics_draw_circle(the_map_view, &carmen_grey, TRUE, draw_point, robot_size);
      carmen_map_graphics_draw_circle(the_map_view, &carmen_black, FALSE, draw_point, robot_size);
      carmen_map_graphics_draw_line(the_map_view, &carmen_black, draw_point, &cursor_pos);
    } else {
      carmen_map_graphics_draw_circle(the_map_view, colour, TRUE, draw_point, robot_size);
      carmen_map_graphics_draw_circle(the_map_view, &carmen_black, FALSE, draw_point, robot_size);
                          ...
                          carmen_navigator_panel_config_t *nav_panel_conf_param)
  {
    ...
    menu_item = gtk_ui_manager_get_widget(ui_manager, "/ui/MainMenu/FileMenu/StopFilming");
    gtk_widget_hide(menu_item);
  ...
  navigator_graphics_change_map(carmen_map_p new_map)
  {
    ...
    sprintf(buffer, "Map: %s", carmen_extract_filename(new_map->config.map_name));
  ...
  navigator_graphics_display_map(float *data, carmen_navigator_map_t type)
  {
    ...
    switch (type) {
      ...
      case CARMEN_LOCALIZE_GMAP_v:
        ...
        flags = CARMEN_GRAPHICS_RESCALE;
        break;
      case CARMEN_PLANNER_MAP_v:
        strcpy(name, "PlannerMap");
        break;
      default:
        return;
    }
  ...
  navigator_graphics_update_display(carmen_traj_point_p new_robot,
                                    ...
                                    int autonomous)
  {
    // AK: paint the intermediate planner map
    static double last_planner_map_update = 0;
    if( FALSE || display == CARMEN_PLANNER_MAP_v ) {
      double curTime = carmen_get_time();
      if( curTime - last_planner_map_update > 0.5 ) {
        last_planner_map_update = curTime;
        //navigator_get_map( CARMEN_PLANNER_MAP_v );
        navigator_get_map( display );
        display_needs_updating = 1;
      }
    }
