...
//    printf( "before >> globalposmsg: pos(%5.3f,%5.3f,%5.3f); odo(%5.3f,%5.3f,%5.3f) -- odo_msg: odo(%5.3f,%5.3f,%5.3f)\n",
//            globalpos.globalpos.x, globalpos.globalpos.y, globalpos.globalpos.theta,
//            globalpos.odometrypos.x, globalpos.odometrypos.y, globalpos.odometrypos.theta,
//            corrected_odometry.x,corrected_odometry.y,corrected_odometry.theta );

    carmen_localize_correct_odometry(&corrected_odometry, &globalpos);

//    printf( "after  >> globalposmsg: pos(%5.3f,%5.3f,%5.3f); odo(%5.3f,%5.3f,%5.3f) -- odo_msg: odo(%5.3f,%5.3f,%5.3f)\n\n",
//            globalpos.globalpos.x, globalpos.globalpos.y, globalpos.globalpos.theta,
//            globalpos.odometrypos.x, globalpos.odometrypos.y, globalpos.odometrypos.theta,
//            corrected_odometry.x,corrected_odometry.y,corrected_odometry.theta );
...
update_positions(void)
{
  ...
  if (carmen_distance(&last_position, &(globalpos.globalpos)) > 2*robot_config.max_t_vel / (globalpos.timestamp - last_timestamp))
...
#if 0  // AK DEBUG: dumped poses for laser offset corrections

  if (globalpos.timestamp <= 0) return;

  printf( "b4 >> lasermsg: laser(%f,%f,%f); robot(%f,%f,%f); global_pos_msg(%f,%f,%f)\n",
          frontlaser.laser_pose.x, frontlaser.laser_pose.y, frontlaser.laser_pose.theta,
          frontlaser.robot_pose.x, frontlaser.robot_pose.y, frontlaser.robot_pose.theta,
          globalpos.globalpos.x,globalpos.globalpos.y,globalpos.globalpos.theta );

  carmen_localize_correct_laser(&frontlaser, &globalpos);

  printf( "after >> lasermsg: laser(%f,%f,%f); robot(%f,%f,%f); global_pos_msg(%f,%f,%f)\n\n",
          frontlaser.laser_pose.x, frontlaser.laser_pose.y, frontlaser.laser_pose.theta,
          frontlaser.robot_pose.x, frontlaser.robot_pose.y, frontlaser.robot_pose.theta,
          globalpos.globalpos.x,globalpos.globalpos.y,globalpos.globalpos.theta );

  if (!(nav_config.map_update_obstacles || nav_config.map_update_freespace) ||
      !autonomous_status)
    return;

  carmen_planner_update_map(&frontlaser, &nav_config, &robot_config);
  carmen_navigator_publish_status();
  carmen_navigator_publish_plan();
...
generate_next_motion_command(void)
{
  ...
  waypoint_status = carmen_planner_next_waypoint(&waypoint, &is_goal, &nav_config);
...
generate_next_motion_command(void)
{
  ...
  waypoint_status = carmen_planner_next_waypoint(&waypoint, &waypoint_index, &is_goal, &nav_config);
  carmen_verbose( " navigator::generate_next_motion_command::waypoint_status: %d\n", waypoint_status );

  if (waypoint_status > 0) /* goal is reached OR unreachable! */
  {
    autonomous_status = 0;

    if( waypoint_status == 2 ) { // AK: added information whether goal is accessible or not
      carmen_navigator_publish_autonomous_stopped(CARMEN_NAVIGATOR_GOAL_NOT_REACHABLE_v);
      carmen_verbose("Autonomous off Motion command %.2f %.2f (goal is NOT reachable!)\n", command.tv, carmen_radians_to_degrees(command.rv));
    } else {
      carmen_navigator_publish_autonomous_stopped(CARMEN_NAVIGATOR_GOAL_REACHED_v);
      carmen_verbose("Autonomous off Motion command %.2f %.2f\n", command.tv, carmen_radians_to_degrees(command.rv));
    }
  ...
  carmen_planner_get_status(&status);
  carmen_verbose( " navigator::generate_next_motion_command::planner_status: points left: %d\n", status.path.length-waypoint_index );

  carmen_robot_follow_trajectory(status.path.points+waypoint_index, status.path.length-waypoint_index, &robot_position);
...
int main(int argc, char **argv)
{
  ...
  carmen_map_apply_offlimits_chunk_to_map(offlimits, num_offlimits_segments, nav_map);
  ...
  carmen_robot_subscribe_frontlaser_message(&frontlaser, (carmen_handler_t)robot_frontlaser_handler, CARMEN_SUBSCRIBE_LATEST);

  carmen_robot_subscribe_rearlaser_message(&rearlaser, (carmen_handler_t)robot_rearlaser_handler, CARMEN_SUBSCRIBE_LATEST);
