...
#define DUMP_FILE_FUNC_LINE if(0) printf( "%s -> %s (%d)!\n", __FILE__, __FUNCTION__, __LINE__ );
#define DUMP_FILE_FUNC_LINE_TEXT(x) if(1) printf( "%s -> %s (%d): %s", __FILE__, __FUNCTION__, __LINE__, x );
...
void navigator_get_map(carmen_navigator_map_t type)
{
  ...
  if (type == CARMEN_LOCALIZE_LMAP_v || type == CARMEN_LOCALIZE_GMAP_v) {
    ...
    for (index = 0; index < new_map.config.x_size*new_map.config.y_size; index++)
  ...
  if (map && (map->config.map_name && new_map.config.map_name) && strcmp(map->config.map_name, new_map.config.map_name) == 0) {
    if (is_graphics_up)
      navigator_graphics_display_map(new_map.complete_map, type);
    // AK: the memory of the new map needs to be freed again if we return in here ...
    free(new_map.complete_map); new_map.complete_map = NULL;
    free(new_map.map); new_map.map = NULL;
    return;
  }
                            ...
                            *navigator_panel_config)
{
  ...
  carmen_param_t param_list[] = {
                                 {"robot", "max_t_vel", CARMEN_PARAM_DOUBLE, &(robot_config->max_t_vel), 1, NULL},
                                 {"robot", "max_r_vel", CARMEN_PARAM_DOUBLE, &(robot_config->max_r_vel), 1, NULL},
                                 ...
                                 {"robot", "acceleration", CARMEN_PARAM_DOUBLE, &(robot_config->acceleration), 1, NULL},
                                 {"robot", "reaction_time", CARMEN_PARAM_DOUBLE, &(robot_config->reaction_time), 0, NULL},
                                 {"robot", "rectangular", CARMEN_PARAM_INT, &(robot_config->rectangular), 1, NULL},
...
int main(int argc, char **argv)
{
  ...
  carmen_navigator_subscribe_status_message(&status, (carmen_handler_t)navigator_status_handler, CARMEN_SUBSCRIBE_LATEST);
  carmen_navigator_subscribe_plan_message(NULL, (carmen_handler_t)navigator_plan_handler, CARMEN_SUBSCRIBE_LATEST);
  /*  carmen_localize_subscribe_people_message(&people_message, (carmen_handler_t)people_handler,CARMEN_SUBSCRIBE_LATEST);*/
  carmen_localize_subscribe_globalpos_message(&globalpos, (carmen_handler_t)globalpos_handler, CARMEN_SUBSCRIBE_LATEST);

  carmen_simulator_subscribe_truepos_message(NULL, (carmen_handler_t)truepos_handler, CARMEN_SUBSCRIBE_LATEST);

  carmen_simulator_subscribe_objects_message(NULL, (carmen_handler_t)objects_handler, CARMEN_SUBSCRIBE_LATEST);

  carmen_map_subscribe_gridmap_update_message(NULL, (carmen_handler_t)map_update_handler, CARMEN_SUBSCRIBE_LATEST);

  err = IPC_defineMsg(CARMEN_NAVIGATOR_DISPLAY_CONFIG_NAME, IPC_VARIABLE_LENGTH, CARMEN_NAVIGATOR_DISPLAY_CONFIG_FMT);
