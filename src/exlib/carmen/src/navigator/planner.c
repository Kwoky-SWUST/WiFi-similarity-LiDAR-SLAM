...
extract_path_from_value_function(void)
{
  ...
  if (goal_is_accessible) {
    map_goal.x = carmen_round(requested_goal.x / carmen_planner_map->config.resolution);
    map_goal.y = carmen_round(requested_goal.y / carmen_planner_map->config.resolution);
    map_goal.map = carmen_planner_map;
  } else {
    map_goal.x = carmen_round(intermediate_goal.x / carmen_planner_map->config.resolution);
    map_goal.y = carmen_round(intermediate_goal.y / carmen_planner_map->config.resolution);
    map_goal.map = carmen_planner_map;
  }
...
static void
compute_cost(int start_index, int end_index, double *cost_along_path, double *min_cost)
{
...
cost_of_path(void)
{
  ...
  total_cost = 0.0;
  for (path_index = 1; path_index < path.length; path_index++)
  {
...
smooth_path(carmen_navigator_config_t *nav_conf)
{
  ...
  while (path.length > 2 && carmen_distance_traj(&robot, carmen_planner_util_get_path_point(1, &path)) < nav_conf->goal_size) {
  ...
  while (path_index < path.length-1)
  {
    ...
    if (cost_along_prev+cost_along_next+1e-6 < new_cost || min_cost_next < new_min_cost || min_cost_prev < new_min_cost)
    {
      path_index++;
    }
    else
...
static int find_nearest_free_point_to_goal(void)
{
  ...
  goal_x = carmen_round(requested_goal.x / carmen_planner_map->config.resolution);
  goal_y = carmen_round(requested_goal.y / carmen_planner_map->config.resolution);
...
plan(carmen_navigator_config_t *nav_conf)
{
  ...
  if (nav_conf->replan_frequency > 0) {
    if (carmen_get_time() - last_plan_time < 1.0/nav_conf->replan_frequency) {
      return;
    }
  }
  ...
  // check whether the robot position has a positive utility value, if the goal is INSIDE
  // an obstacle region, this value will be negative!
...
regenerate_trajectory(carmen_navigator_config_t *nav_conf)
{
  ...
  if (extract_path_from_value_function() < 0)
    ...
    if( carmen_carp_get_verbose() ) { // dump path points
                            ...
                            carmen_navigator_config_t *nav_conf)
{
  ...
  if (new_position->x < 0 || new_position->y < 0 ||
      new_position->x > carmen_planner_map->config.resolution * carmen_planner_map->config.x_size ||
      new_position->y > carmen_planner_map->config.resolution*carmen_planner_map->config.y_size)
    return 0;

  robot = *new_position;

  if (!first_time && carmen_distance_traj(new_position, &old_position) < carmen_planner_map->config.resolution)
    return 0;
...
carmen_planner_get_status(carmen_planner_status_p status)
{
  ...
  if (status->path.length > 0) {
    status->path.points = (carmen_traj_point_p)calloc(status->path.length, sizeof(carmen_traj_point_t));
...
// Add a check for planners with goals -- if we're just tracking' then there is no goal
                             ...
                             carmen_navigator_config_t *nav_conf)
{
  ...
  if (delta_dist < nav_conf->goal_size && path.length - next_point == 1) {
    if( !goal_is_accessible ) // AK: added information whether goal is accessible or not
      return 2;
...
carmen_planner_get_map_message(carmen_navigator_map_t map_type)
{
  ...
  reply = (carmen_navigator_map_message *)calloc(1, sizeof(carmen_navigator_map_message));
  ...
  if (map_type == CARMEN_NAVIGATOR_UTILITY_v || map_type == CARMEN_NAVIGATOR_COST_v) // AK: cost pointer/memory should also be freed again ...
    free(map_ptr);
  return reply;
}
