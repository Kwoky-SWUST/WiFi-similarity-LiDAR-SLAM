...
int PozyxClass::doRanging(uint16_t destination, device_range_t *range){
  ...
  //Serial.println("status after regFunction ");Serial.println(status);
  if (status == POZYX_SUCCESS && waitForFlag(POZYX_INT_STATUS_FUNC, POZYX_DELAY_INTERRUPT)){
   ...
   //Serial.print("after get "); Serial.println(status);
    return status;
  }
  else{
  ...
  //Serial.print("Status from header\t"); Serial.println(status);
  return status;
}
